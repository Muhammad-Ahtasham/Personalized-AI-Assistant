...
function HomePageContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.message.includes('Authentication required')) {
        showError(
          "Please sign in to generate learning plans. Click the 'Sign In' button in the top right."
        );
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      ...
      if (error.message.includes('Authentication required')) {
        showError(
          "Please sign in to generate quizzes. Click the 'Sign In' button in the top right."
        );
  ...
  const handleSubmitQuiz = async () => {
    ...
    // Save quiz result if user is signed in
    if (user) {
      const score = quizData.quiz.reduce(
        (acc, q, i) => acc + (quizData.userAnswers[i] === q.answer ? 1 : 0),
        0
      );
      const saveRes = await fetch('/api/save-quiz-result', {
        method: 'POST',
          ...
          {!user && (
            <div className="mb-6 sm:mb-8 p-4 sm:p-6 card-dark">
              ...
              <div className="mt-4 text-center text-xs sm:text-sm text-muted-foreground">
                ...
                <a href="/sign-up" className="text-green-accent hover:underline">
                  Sign up with email
                </a>{' '}
                or{' '}
                <a href="/face-sign-up" className="text-green-accent hover:underline">
                  sign up with face
                </a>
              </div>
            </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-yellow-accent font-medium">
                    Saved content for:{' '}
                    <span className="text-white">{planData?.topic || quizData?.topic}</span>
                  </span>
                </div>
